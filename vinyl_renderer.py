...
def render_vinyl(
    ...
    background: tuple[int, int, int, int] = (0, 0, 0, 0),   # ← 透過に変更
) -> Image.Image | None:
    """
    ...
    for layer in layers:
        ...
        is_mask = int(layer.get("mask", 0)) == 1

        # このレイヤー専用の透明バッファにシルエットを描画
        layer_buf = Image.new("RGBA", (img_w, img_h), (0, 0, 0, 0))
        layer_draw = ImageDraw.Draw(layer_buf)
        fill_color = (255, 255, 255, 255) if is_mask else (*shape.color[:3], 255)
        for (x0, y0), (x1, y1), (x2, y2) in shape.triangles:
            p0 = (tx(x0), ty(y0))
            p1 = (tx(x1), ty(y1))
            p2 = (tx(x2), ty(y2))
            layer_draw.polygon([p0, p1, p2], fill=fill_color)

        if is_mask:
            # 形状の重なる範囲だけ、これまでの合成結果のアルファを0にする（くり抜き）
            mask_alpha = layer_buf.getchannel("A")
            inv_mask = mask_alpha.point(lambda v: 255 - v)
            img_alpha = img.getchannel("A")
            new_alpha = ImageChops.multiply(img_alpha, inv_mask)
            img.putalpha(new_alpha)
            continue  # mask層自体は描画しない（純粋にくり抜き用）
